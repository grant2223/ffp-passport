...
(function () {
  ...
  function wrapWrites() {
    ...
    // ─── FOOD PHOTOS — capture (camera/gallery) → upload → member_food_photos; visual food diary ───
    CalorieTracker.addFoodPhoto = function () {
      if (!currentUserId) { if (window.showToast) showToast('Sign in to add food photos', 'error'); return; }
      if (!window.FFPUpload || !window.FFPUpload.pick) { if (window.showToast) showToast('Photo upload unavailable', 'error'); return; }
      window.FFPUpload.pick({
        bucket: 'activity-photos', key: 'food/' + currentUserId + '-' + Date.now() + '.jpg', aspect: 1, outW: 900, outH: 900, title: 'Food photo',
        onDone: async function (url) {
          try {
            var res = await window.supabase.from('member_food_photos').insert({ member_id: currentUserId, photo_url: url, logged_at: ctLoggedAt() }).select('id').single();
            if (res.error) { console.error('[FFP CT] food photo insert:', res.error); if (window.showToast) showToast('Could not save photo', 'error'); return; }
            await CalorieTracker.loadFoodPhotos();
            CalorieTracker.render();
            if (window.showToast) showToast('Food photo saved', 'success');
          } catch (e) { console.error('[FFP CT] food photo:', e); if (window.showToast) showToast('Could not save photo', 'error'); }
        },
        onError: function () {}
      });
    };
    CalorieTracker.deleteFoodPhoto = async function (id) {
      if (!id || !currentUserId) return;
      CalorieTracker._foodPhotosToday = (CalorieTracker._foodPhotosToday || []).filter(function (p) { return p.id !== id; });
      CalorieTracker._foodPhotosWeek = (CalorieTracker._foodPhotosWeek || []).filter(function (p) { return p.id !== id; });
      CalorieTracker.renderFoodPhotos('today');
      try { await window.supabase.from('member_food_photos').delete().eq('id', id).eq('member_id', currentUserId); } catch (e) {}
    };
    CalorieTracker.loadFoodPhotos = async function () {
      if (!currentUserId) { CalorieTracker._foodPhotosToday = []; CalorieTracker._foodPhotosWeek = []; return; }
      try {
        var since = new Date(Date.now() - 7 * 86400000).toISOString();
        var res = await window.supabase.from('member_food_photos').select('id, photo_url, logged_at').eq('member_id', currentUserId).gte('logged_at', since).order('logged_at', { ascending: false });
        var rows = (res && res.data) ? res.data : [];
        CalorieTracker._foodPhotosWeek = rows;
        var viewYmd = CalorieTracker._ymd(CalorieTracker.viewDate || CalorieTracker._todayMidnight());
        CalorieTracker._foodPhotosToday = rows.filter(function (p) { try { var d = new Date(p.logged_at); d.setHours(0, 0, 0, 0); return CalorieTracker._ymd(d) === viewYmd; } catch (e) { return false; } });
      } catch (e) { CalorieTracker._foodPhotosToday = []; CalorieTracker._foodPhotosWeek = []; }
    };
  ...
  CalorieTracker.loadDayData = function (date) {
    ...
    window.supabase.from('food_logs')
      ...
      .then(function (res) {
        ...
        CalorieTracker.meals = meals;
        // re-filter the day's food photos from the already-loaded 7-day set
        try { var dy = CalorieTracker._ymd(start); CalorieTracker._foodPhotosToday = (CalorieTracker._foodPhotosWeek || []).filter(function (p) { try { var d = new Date(p.logged_at); d.setHours(0, 0, 0, 0); return CalorieTracker._ymd(d) === dy; } catch (e) { return false; } }); } catch (e) {}
