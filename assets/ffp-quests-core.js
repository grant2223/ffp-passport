/* FFP Quests — core module — v2 (2026-06-09)
   v2: INVITE FRIENDS — member quests now show "Invite friends to join me" → a connections picker
       (member_connections_list, accepted only) → POST /api/quests/:id/invite → each invitee gets a
       bell + push with a deep link. init() also opens ?quest=<id> from an invite link. The Edit-quest
       button now works because the backend returns created_by (v87) — and Edit is where the photo is
       swapped, so this also unblocks changing a quest's photo.
   ── v1 (2026-06-08) ──
...
window.Quests = {
  ...
  async init() {
    ...
    await this.load();
    // Deep link from an invite push/notification: ?quest=<id> opens that quest. Runs once.
    try {
      var qp = new URLSearchParams(window.location.search).get('quest');
      if (qp && !this._deepLinked) { this._deepLinked = true; this.openDetail(qp); }
    } catch (e) {}
  },
  ...
  // ── Invite friends to "join me" on a quest (in-app bell + push) ──
  async openInvite(questId) {
    if (document.getElementById('qinv-overlay')) return;
    var mid = this.memberId();
    if (!mid || !window.supabase) { showToast('Please sign in again', 'error'); return; }
    this.qcInjectCss();
    window._ffpInv = { quest: questId, picked: {}, people: [] };
    var ov = document.createElement('div');
    ov.id = 'qinv-overlay';
    ...
    ov.innerHTML =
      '<div style="padding:16px 18px;border-bottom:1px solid rgba(43,168,224,0.2);display:flex;align-items:center;gap:10px;flex-shrink:0;">' +
        ...
        '<div style="font-size:16px;font-weight:800;color:#e8eef4;">Invite friends to join</div>' +
      '</div>' +
    ...
    try {
      ...
      // accepted (mutual) connections only — the people you can ask to join you
      arr = arr.filter(function (p) { return p && p.mutual; });
      window._ffpInv.people = arr;
      this.renderInviteList('');
    } catch (e) {
      var h = document.getElementById('qinv-list');
      if (h) h.innerHTML = '<div style="text-align:center;color:#8a99a8;padding:30px;">Couldn’t load your connections.</div>';
    }
  },

  renderInviteList(q) {
    var st = window._ffpInv || {}; var people = st.people || [];
    q = (q || '').trim().toLowerCase();
    ...
    if (!people.length) {
      host.innerHTML = '<div style="text-align:center;color:#8a99a8;padding:30px;line-height:1.6;">' +
        ((st.people && st.people.length) ? 'No connections match.' : 'You have no connections yet — connect with people first, then invite them here.') + '</div>';
      return;
    }
    ...
    host.innerHTML = people.map(function (p) {
      ...
      var av = p.photo
        ...
        : '<div class="qc-venue-av" style="border-radius:50%;">' + escHtml((p.name || 'M').charAt(0).toUpperCase()) + '</div>';
      var area = [p.city, p.country].filter(Boolean).join(' · ');
      return '<div class="qc-venue-card pickable' + (picked ? ' picked' : '') + '" onclick="Quests.toggleInvite(\'' + p.id + '\')">' +
        av +
        '<div class="qc-venue-meta"><div class="qc-venue-nm">' + escHtml(p.name || 'Member') + '</div>' +
        (area ? '<div class="qc-venue-area">' + escHtml(area) + '</div>' : '') + '</div>' +
        '<span class="material-icons qc-venue-check">' + (picked ? 'check_circle' : 'radio_button_unchecked') + '</span></div>';
    }).join('');
    var btn = document.getElementById('qinv-send');
    if (btn) btn.textContent = nPicked ? ('Send ' + nPicked + ' invite' + (nPicked === 1 ? '' : 's')) : 'Send invites';
  },

  inviteFilter(q) { this.renderInviteList(q || ''); },

  toggleInvite(id) {
    var st = window._ffpInv || {}; if (!st.picked) st.picked = {};
    st.picked[id] = !st.picked[id]; window._ffpInv = st;
    var s = document.getElementById('qinv-search'); this.renderInviteList(s ? s.value : '');
  },
  ...
  async sendInvites() {
    ...
    try {
      var res = await fetch(this.base + '/api/quests/' + st.quest + '/invite', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ from_member_id: mid, to_member_ids: ids })
      });
      var json = await res.json();
      if (!res.ok || !json.success) throw new Error(json.error || 'Invite failed');
      var n = (json.sent != null ? json.sent : ids.length);
      this.closeInvite();
      showToast(n + ' invite' + (n === 1 ? '' : 's') + ' sent — they’ll get a notification 🎉', 'success');
    } catch (e) {
      console.error('[Quests] invite:', e);
      showToast(e.message || 'Could not send invites', 'error');
