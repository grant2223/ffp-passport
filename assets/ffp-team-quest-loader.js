...
(function () {
  ...
  W.FFPTeamQuest.open = function (questId, opts) {
    ...
    opts = opts || {};
    S.q = questId; S.metric = opts.metric || 'avg'; S.min = opts.minMembers || 10; S.title = opts.title || 'Most active team';
  ...
  function paintBoard() {
    ...
    if (myRow) {
      ...
      hero = '<div style="position:relative;padding:18px 20px 20px;background:linear-gradient(160deg,#123a52 0%,#0b2233 60%,#0a1825 100%);">' +
        ...
        '<span onclick="FFPTeamQuest.close()" style="cursor:pointer;color:#9fc0d4;font-size:22px;line-height:1;">&times;</span></div>' +
      ...
      hero = '<div style="position:relative;padding:18px 20px 20px;background:linear-gradient(160deg,#123a52,#0a1825);">' +
        ...
        '<div style="font-size:13px;color:#9fc0d4;margin-top:6px;">Join a team, or create one in the FFP Pro app — then your activity counts for the team.</div></div>';
    }
