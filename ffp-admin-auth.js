...
(function () {
  ...
  // v6: grantAccess now reflects the logged-in admin email/role in the sidebar.
  // ─── Supabase actions ───
  ...
  async function checkAdminAndProceed() {
    try {
      ...
      grantAccess(adminRes.data.role, user.email);
    } catch (e) {
  ...
  function grantAccess(role, email) {
    // Tell the dashboard who's logged in (in case it wants to display it)
    window.FFP_ADMIN = { role: role, email: email || null };
    // v6: reflect the ACTUAL logged-in admin in the sidebar (was hardcoded "Grant Goes").
    try {
      var roleText = String(role || 'admin').replace(/_/g, ' ').replace(/\b\w/g, function (c) { return c.toUpperCase(); });
      var nm = document.getElementById('ffp-admin-name'); if (nm) nm.textContent = email || 'Admin';
      var rl = document.getElementById('ffp-admin-role'); if (rl) rl.textContent = roleText;
      var av = document.getElementById('ffp-admin-avatar'); if (av) av.textContent = ((email || 'A')[0] || 'A').toUpperCase();
      var id = document.getElementById('ffp-admin-identity'); if (id) id.title = 'Signed in as ' + (email || 'admin');
    } catch (e) {}
