/* FFP Connections Feed Loader — v1 (2026-06-07)
...
(function () {
  ...
  async function load() {
    var root = document.getElementById('conn-panel-root');
    if (!root) return;
    if (!window.supabase) { setTimeout(load, 250); return; }
    var me = memberId();
    if (!me) { setTimeout(load, 300); return; }
  ...
  function init() {
    ...
    if (btn && !btn._cfHooked) { btn._cfHooked = true; btn.addEventListener('click', function () { setTimeout(load, 60); }); }
    var p = document.getElementById('panel-meet');
